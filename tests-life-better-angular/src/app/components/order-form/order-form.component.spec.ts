...
describe("Order", () => {
  describe("#first-name input", () => {
    ...
    it("should proper validate blur", async () => {
      ...
      const validationMessage = await screen.findByText(firstNameRequiredValidationMessage)
  ...
  describe("#email input", () => {
    ...
    it("should validate shape of email when bad format", async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.queryByText(emailShapeValidationMessage)).toBeInTheDocument();
      });
    });
  ...
  describe("Submission", () => {
    ...
    it("should validate whole form when submit", async () => {
      ...
      expect(await screen.findByText(firstNameRequiredValidationMessage)).toBeDefined();
      expect(await screen.findByLabelText(firstNameLabel)).toBeInvalid();

      expect(await screen.findByText(lastNameRequiredValidationMessage)).toBeDefined();
      expect(await screen.findByLabelText(lastNameLabel)).toBeInvalid();

      expect(await screen.findByText(postalCodeRequiredValidationMessage)).toBeDefined();
      expect(await screen.findByLabelText(postalCodeLabel)).toBeInvalid();

      expect(await screen.findByText(addressRequiredValidationMessage)).toBeDefined();
